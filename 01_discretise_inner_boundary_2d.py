...
def discretise_points(grid_coords, coords_rc, x, y, nx=[], ny=[]):
    if list(nx) == []:
        rcs = []
        already_seen = set()
...
coords = rasterise_to_coords(x, y, grid)
x_disc, y_disc = transform_to_xy(coords, grid)
vector_field = rasterise_vectors(x, y, nx, ny, grid)

assert vector_field.shape[0] == x_disc.size


def make_boundary_and_plot(n, ax):
    x, y, nx, ny = parameteric_circle(n)
    coords = rasterise_to_coords(x, y, grid)
    x_disc, y_disc = transform_to_xy(coords, grid)
    vector_field = rasterise_vectors(x, y, nx, ny, grid)
    ax.plot(x, y, "o")
    ax.plot(x_disc, y_disc, "-x")
    ax.quiver(x_disc, y_disc, vector_field[:, 0], vector_field[:, 1])
